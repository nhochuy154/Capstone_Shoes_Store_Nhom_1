...
const getProductDetail = async (productId) => {
    try {
        ...
        if (error.response) {
            ...
            showError(`Lỗi server: ${error.response.data.message || "Không thể tải thông tin sản phẩm"}`);
...
const renderProductDetail = (product) => {
    try {
        ...
        const descriptionElement = document.querySelector(".product-detail__description p");
        if (descriptionElement) descriptionElement.textContent = product.shortDescription;
        ...
        const skuElement = document.querySelector(".product-detail__availability .sku");
        ...
        const stockElement = document.querySelector(".product-detail__availability .stock");
        if (stockElement) {
        ...
        // Hiển thị categories
        const categoriesElement = document.querySelector(".product-detail__categories");
...
const handleWishlist = () => {
    ...
    // Hiển thị thông báo
    Toastify({
        text: wishlistBtn.classList.contains("active") ? "Đã thêm vào yêu thích" : "Đã xóa khỏi yêu thích",
        duration: 3000,
        gravity: "top",
        position: "right",
        style: {
...
const initProductDetail = () => {
    ...
    document.querySelector(".quantity-selector input").addEventListener("change", handleQuantityChange);
    document.querySelector(".quantity-selector").addEventListener("click", handleQuantityButton);
    document.querySelector(".wishlist-btn").addEventListener("click", handleWishlist);
    document.querySelector(".color-swatches").addEventListener("click", handleColorSelect);
...
const renderRelatedProducts = (products) => {
    const relatedProductsContainer = document.querySelector(".related-products__grid");
